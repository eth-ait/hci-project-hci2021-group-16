1. Create a new Vue3 project with vue.cli
2. Replace source folder
3. npm run serve (to serve file in browser)


To run Test:
1. Enter user ID in variables

2. set TestType to 0 for Buster/Checkbox test
2.1 refresh page befor starting (Timer starts directly after refresh)
2.2 copy data displayed at end of test on the top of the page to a seperate file

3. set TestType to 1 for three different input methods for Buster test
3.1 refresh page befor starting (Timer starts directly after refresh)
3.2 copy data displayed at end of test on the top of the page to a seperate file